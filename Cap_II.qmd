...
![Tabla 1. Datos estructurados no ordenados](Imagenes/Tabla 1.png "Empresas Nivel Central"){width="90%"}
...
# Base de datos de matriculados a la Universidad

MatriculadosPV <- UnalData::Matriculados %>% select(-PROGRAMA, YEAR, SEMESTRE, DEP_PROC, CIU_PROC, SEXO, CAT_EDAD, ESTRATO, MOD_ADM, AREAC_SNIES, NACIONALIDAD, FACULTAD, TIPO_NIVEL, EDAD_MOD) %>%
  filter(MAT_PVEZ == "Sí") %>%
...
color <- function(n_colores){
  salida <- colorRampPalette(c('#A8A8A8','#4D4D4D'))(n_colores)
  return(salida)
}
```
...
Agregar(SEXO ~ YEAR + SEMESTRE, 
                    ...
                    datos = MatriculadosPV,
                    textNA = "Sin información") %>% 
           Plot.Series(categoria = "SEXO", 
              titulo = "Evolución del número de estudiantes matriculados por primera vez por sexo",
              labelY = "Número de estudiantes",
              ylim = c(0,NaN),
              libreria = c("highcharter"), 
              colores = color(length(unique(MatriculadosPV$SEXO))),
              estilo = list(hc.Tema = tema, hc.Slider = FALSE, hc.Credits = ""))
```
...
Agregar(SEXO ~ YEAR + SEMESTRE, 
        ...
        Plot.Torta(categoria  = "SEXO",
           ano        = "2022",
           periodo    = "2",
           titulo     = "Distribución de estudiantes matriculados por primera vez por sexo",
           addPeriodo = TRUE,
...
Hombres <- MatriculadosPV %>% group_by(YEAR, SEMESTRE, SEXO) %>% summarise(Total=n()) %>% filter(SEXO == "Hombres") %>% ungroup()

Mujeres <- MatriculadosPV %>% group_by(YEAR, SEMESTRE, SEXO) %>% summarise(Total=n()) %>% filter(SEXO == "Mujeres") %>% ungroup()

MatPV_Tabla <- Hombres[,c(1,2)] %>% mutate(Hombres = Hombres$Total, Mujeres = Mujeres$Total, Total = Hombres + Mujeres)

#Tabla en HTML
UnalR::Tabla.General(datos = MatPV_Tabla,
                     encabezado = "Tabla 3. Total estudiantes matriculados por primera vez por sexo", 
                     colorHead = "lightgray",
                     colNames = c("Año","Semestre","Hombres","Mujeres","Total"),
                     filtros = TRUE,
                     scrollX = FALSE)
...
MatriculadosPV %>% filter(YEAR == ano, SEMESTRE == semestre) %>%
  Plot.Mapa(depto         = COD_DEP_PROC,
            mpio          = COD_CIU_PROC,
            estadistico   = "Conteo",
            tipo          = "SiNoMpios",
            titulo        = paste("Matriculados por primera vez", etiqueta),
